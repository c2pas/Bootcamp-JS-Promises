...
const userData = new Promise(function (resolve, reject) {
    ...
    const changeName = sortedUsers.map(function (user) {
        const fullName = user.fullName.split('_');
        const firstName1st = fullName[0].charAt(0).toUpperCase()
        const lastName1st = fullName[1].charAt(0).toUpperCase()
        const firstname = firstName1st + fullName[0].slice(1);
        const lastname = lastName1st + fullName[1].slice(1);
        return {
            ...user,
    ...
    const adult =  changeName.filter(function(user) {
        return user.age >= 18;
    });
    console.log(adult);
    resolve(adult);
...
userData.then(function (data) {
    const salaries = data.reduce((acc, cur) => acc + cur.salary, 0);
